...
class Calculator:
    def __init__(self, master):
        ...
        master.configure(bg="LightCyan2")
        self.total = tk.StringVar()

        self.entry = tk.Entry(master,bd=20 ,bg = "powder blue",justify="right",textvariable=self.total, font=("Helvetica", 20))
    ...
    def create_buttons(self):
        ...
        for i, row in enumerate(button_list):
            for j, button_text in enumerate(row):
                button = tk.Button(
                    self.master,bd = 5, text=button_text, width=5, height=3, font=("Helvetica", 16),
